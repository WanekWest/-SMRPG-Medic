...
ConVar g_hCVIncrease, g_hCVInterval, g_hCVRadius, g_hCVRadiusIncrease, hexpval, hvarexpgive, hroundstartdel;

bool g_bIsCstrike, g_delay = false;

Handle g_hMedicTimer;

int g_hexpval, g_hvarexpgive;

float g_hroundstartdel;
...
public void EventRoundStart(Event event, const char[] name, bool dbc)
{
	g_delay = true;
	CreateTimer(g_hroundstartdel, delaykin, _, TIMER_FLAG_NO_MAPCHANGE);
}

Action delaykin(Handle hTimer, any UserId)
{
	g_delay = false;
}
...
public void OnLibraryAdded(const char[] name)
{
	...
	if(StrEqual(name, "smrpg"))
	{
		...
		hvarexpgive = SMRPG_CreateUpgradeConVar(UPGRADE_SHORTNAME, "smrpg_medic_exp_give_var", "0", "0 - static bonus = value; 1 - bonus = value(under)*level.", _, true, 0.0);
		hvarexpgive.AddChangeHook(OnVarchangemed);
		g_hvarexpgive = hvarexpgive.IntValue;

		hexpval = SMRPG_CreateUpgradeConVar(UPGRADE_SHORTNAME, "smrpg_medic_exp_bonus", "100", "How many bonus exp give for client(For healing mates)?.", _, true, 0.0);
		hexpval.AddChangeHook(OnMedExpChange);
		g_hexpval = hexpval.IntValue;

		hroundstartdel = SMRPG_CreateUpgradeConVar(UPGRADE_SHORTNAME, "smrpg_round_med_dal", "10.0", "How many sec wait in new round before giving exp?.", _, true, 0.0);
		hroundstartdel.AddChangeHook(OnMedRoDelCh);
		g_hroundstartdel = hroundstartdel.FloatValue;
	}
}

public void OnMedExpChange(ConVar hCvar, const char[] szOldValue, const char[] szNewValue)
{
	g_hexpval = hCvar.IntValue;
}

public void OnVarchangemed(ConVar hCvar, const char[] szOldValue, const char[] szNewValue)
{
	g_hvarexpgive = hCvar.IntValue;
}

public void OnMedRoDelCh(ConVar hCvar, const char[] szOldValue, const char[] szNewValue)
{
	g_hroundstartdel = hCvar.FloatValue;
}
		...
		for(int m=1;m<=MaxClients;m++)
		{
			...
			if(iNewHP < iMaxHealth)
			{
				if(!g_delay)
				{
					if(g_hvarexpgive == 0)
					{
						SMRPG_AddClientExperience(i, g_hexpval, "healing", false, -1);
					}
					else
					{
						int exp = g_hexpval * iLevel;
						SMRPG_AddClientExperience(i, exp, "healing", false, -1);
					}
				}
